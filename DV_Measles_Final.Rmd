...
```
# 3) Lab confirmed case ratio
```{r lab_confirmation, echo=FALSE}

# Ensure lab_ratio exists and add short region names
measles_clean <- measles_clean %>%
  mutate(
    lab_ratio = ifelse(measles_total > 0, measles_lab_confirmed / measles_total, NA),
    region_full = region_names[region]
  )

# Mean lab ratio by region
lab_ratio_region <- measles_clean %>%
  group_by(region_full) %>%
  summarise(
    mean_lab_ratio = mean(lab_ratio, na.rm = TRUE),
    total_cases = sum(measles_total, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(desc(mean_lab_ratio))

# Vertical bar plot with short region names
lab_ratio1 <- ggplot(lab_ratio_region, aes(x = reorder(region_full, mean_lab_ratio), y = mean_lab_ratio, fill = region_full)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = paste0(sprintf("%.1f", mean_lab_ratio * 100), "%")),
            vjust = -0.5, color = "black", size = 3.5) +
  scale_x_discrete(labels = function(x) gsub(" ", "\n", x)) + 
  scale_y_continuous(labels = percent_format(accuracy = 1),
                     expand = expansion(mult = c(0, 0.1))) +
  labs(
    title = "Lab-Confirmed / Total Measles Ratio by WHO Region (2015–2024)",
    x = "",
    y = ""
  ) +
  theme_minimal(base_size = 13) +
...
lab_ratio1
# ggsave("06_labregion_measles_plot.png", plot = lab_ratio1, width = 8, height = 5, dpi = 300)

# Bottom 10 countries
bottom10_countries <- measles_clean %>%
  group_by(country, region_full) %>%
  summarise(
    mean_lab_ratio = mean(lab_ratio, na.rm = TRUE),
    total_cases = sum(measles_total, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(mean_lab_ratio) %>%
  slice_head(n = 10)  # bottom 10 only

# Horizontal bar plot
lab_ratio2 <- ggplot(bottom10_countries, aes(x = reorder(country, mean_lab_ratio), y = mean_lab_ratio, fill = region_full)) +
  geom_col() +
  geom_text(aes(label = paste0(round(mean_lab_ratio * 100, 1), "%")),
            hjust = -0.1, color = "black", size = 3.5) + 
  scale_y_continuous(labels = percent_format(accuracy = 1),
                     expand = expansion(mult = c(0, 0.2))) +
  labs(
    title = "Lab-Confirmed / Total Measles Ratio, Lowest 10 Countries (2015–2024)",
    x = "",
    y = "",
    fill = "Region"
  ) +
...
lab_ratio2
# ggsave("07_labcountry_measles_plot.png", plot = lab_ratio2, width = 8, height = 5, dpi = 300)
```


# 4) Vaccination coverage (WUENIC), countries & WHO regions
...
# 5) Dropout (MCV1 − MCV2) and summaries
...
p_dropout_facets
# ggsave("08_dropout_region_trend.png", p_dropout_facets, width = 8, height = 5, dpi = 300)
```

# 6) Merge incidence + vaccination + dropout

```{r merge-master}

measles_vax_full <- measles_clean %>%
  left_join(
    vax_clean %>% filter(antigen == "MCV2") %>% 
      select(country, year, MCV2 = coverage_value),
    by = c("country", "year")
  ) %>%
  left_join(
    vax_clean %>% filter(antigen == "MCV1") %>% 
...
# 7) Incidence vs vaccination visuals
...
p_mcv2_binned
# ggsave("09_incidence_vs_mcv2.png", p_mcv2_binned, width = 8, height = 5, dpi = 300)
```
...
p_overlay
# ggsave("10_regional_incidence_vs_coverage.png", p_overlay, width = 9, height = 6, dpi = 300)
```
...
-   06_labregion_measles_plot.png\
-   07_labcountry_measles_plot.png\
-   08_dropout_region_trend.png\
-   09_incidence_vs_mcv2.png\
-   10_regional_incidence_vs_coverage.png\
